...
function getPristineRefPoints(zoomValue, mx, my) {
    if (zoomValue != 1) {
        return new Point(getPix(mx / zoomValue), getPix(my / zoomValue));
    } else {
...
function mouseDown(e) {
    ...
    // myShape.cStartPoint = getPristineRefPoints(iModel.scale, mx, my)
...
function mouseUp(e) {
    ...
    // myShape.cEndPoint = getPristineRefPoints(iModel.scale, mx, my)
    if (drawShape) {
        myShape.cStartPoint = getNormalScalePoints(myShape.startPoint)
        myShape.cEndPoint = getNormalScalePoints(myShape.endPoint)
    }
...
function getNormalScalePoints(point = new Point(0, 0)) {
    return getPristineRefPoints(iModel.scale, point.x, point.y)
}