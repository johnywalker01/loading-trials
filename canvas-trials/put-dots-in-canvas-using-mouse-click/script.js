...
function getRotatedPoint(coords, angle) {
    var x2 = coords.x;
    var y2 = coords.y;
...
function mouseDown(e) {
    ...
    let cmx = Math.floor(e.offsetX || (e.pageX - offsetX));
    let cmy = Math.floor(e.offsetY || (e.pageY - offsetY));

    let pos = new Position(cmx, cmy);
    if (model.angleInRad != 0) {
        var angle = (model.angleInRad * -1);
        pos = getRotatedPoint(pos, angle);
 ...
 * @param {*} my 
 */
function drawDot(mx, my, color = "#ff3333") {
    model.ctx.beginPath();
    model.ctx.fillStyle = color;
